...
docker build -t titovanton/samba-node:brand-new https://raw.githubusercontent.com/titovanton/bicycle-docker/master/samba-node/Dockerfile
...
last=$(sudo docker ps -l -q)
docker commit $last titovanton/samba-node:$USER_NAME